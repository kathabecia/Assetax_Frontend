...
const form_profile_password = document.getElementById("form_profile_password");

form_profile_password.onsubmit = async (e) => {
  e.preventDefault();

  // Disable button
  document.querySelector("#form_profile_password button[type = 'submit']").disabled = true;
  document.querySelector("#form_profile_password button[type = 'submit']").innerHTML = 
  ...
  const formData = new FormData(form_profile_password);
  ...
  if (for_update_id) {
    ...
    response = await fetch(
      backendURL + "/api/user/password/" + for_update_id,
      {
  ...
  if (response.ok) {
    ...
    // Reset Form
    form_profile_password.reset();

    successNotification("Successfully" + (for_update_id == "" ? " created" : " updated") + " password.", 10);
...
//   for_update_id = "";
  document.querySelector("#form_profile_password button[type='submit']").disabled = false;
  document.querySelector("#form_profile_password button[type='submit']").innerHTML = "Change Password";
